...
const useShows = () => {
        ...
        for (const s of shows) {
            ...
            if (!map.has(t.id)) {
                map.set(t.id, {
                    ...
                    box_score: s.box_score || 0,
                    attendance: s.box_score ? s.attendance || 0 : 0,
                    total_nights: s.nights,
                    reported_nights: s.box_score ? s.nights : 0,
                    sum_venues: 1,
                    sum_sold: s.box_score ? s.sold_percentage || 0 : 0,
                    sum_price: s.box_score && s.attendance ? s.box_score / s.attendance : 0
                })
                continue
            }
    ...
    const getAllShowsByGroupId = async (id: number): Promise<void> => {
        ...
        const { data, error } = await supabase
            .from('shows')
            .select("*, tour:tour(*), group:group(*), venue:venue(*, city:city(*, country:country(*, continent:continent(*))))")
            .eq('group', id)

        if (error) {
            setLoading(false)
            setApiError({ isError: true, message: error.message })
            console.error("[ShowsHook] Error fetching group's concert shows: ", error)
